...
*  2.1a  09/04/09     DW        Modified for RTOS Demo (TICK->MIWITICK)
********************************************************************/
...
void P2PTasks(void)
{
    ...
    { 
        ...
        rxMessage.flags.bits.command = (MACRxPacket.flags.bits.packetType == (unsigned) PACKET_TYPE_COMMAND) ? 1:0;
        ...
        if( rxMessage.flags.bits.command )
        {
            ...
            switch( rxMessage.Payload[0] )
            {
                    ...
                    case CMD_P2P_CONNECTION_REQUEST:
                            ...
                            // connection request
                                    ...
                                    status == STATUS_EXISTS )
                                {
                                    MiApp_WriteData(P2PCapacityInfo);
                                        ...
                                        for(i = 0; i < (unsigned) ADDITIONAL_NODE_ID_SIZE; i++)
                                        {
                        ...
                        case CMD_P2P_CONNECTION_REMOVAL_REQUEST:
                            {         
                                ...
                                for(i = 0; i < (unsigned) CONNECTION_SIZE; i++)
                                {
                    ...
                    case CMD_P2P_CONNECTION_RESPONSE:
                        ...
                        {
                            if( rxMessage.Payload[1] == (unsigned) STATUS_SUCCESS )
                            {
                                for(i = 0; i < (unsigned) CONNECTION_SIZE; i++)
                                {
                                    // if the record is valid
        ...
        if( rxMessage.PayloadSize == 0u || P2PStatus.bits.SearchConnection || P2PStatus.bits.Resync )
        {
...
BOOL    isSameAddress(INPUT BYTE *Address1, INPUT BYTE *Address2)
{
    ...
    for(i = 0; i < (unsigned) MY_ADDRESS_LENGTH; i++)
    {
    ...
    BYTE    MiApp_EstablishConnection(INPUT BYTE ActiveScanIndex, INPUT BYTE Mode)
    {
        ...
        if( Mode == (unsigned) CONN_MODE_INDIRECT )
        {
            return 0xFF;
        }
...
{ 
    ...
    void DumpConnection(INPUT BYTE index)
    {
        ...
        if( index < (unsigned) CONNECTION_SIZE )
        {
            ...
            if( ConnectionTable[index].status.bits.isValid )
            {
                ...
                for(i = 0; i < 8u; i++)
                {
                    if((i < (unsigned) MY_ADDRESS_LENGTH) && ConnectionTable[index].Address[MY_ADDRESS_LENGTH-1-i])
                    {
        ...
        else
        {
            ...
            for(i = 0; i < (unsigned) CONNECTION_SIZE; i++)
            {
                ...
                if( ConnectionTable[i].status.bits.isValid )
                {
                    ...
                    for(j = 0; j < 8u; j++)
                    {
                        if( j < (unsigned) MY_ADDRESS_LENGTH )
                        {
                    ...
                    Printf("            ");
                    #if ADDITIONAL_NODE_ID_SIZE > 0
                        for(j = 0; j < (unsigned) ADDITIONAL_NODE_ID_SIZE; j++)
                        {
    ...
    BYTE AddConnection(void)
    {
        ...
        {
            if( ConnMode > (unsigned) ENABLE_PREV_CONN )
            {
                return status;
            }
